/*
 * Adapted from Betaflight blackbox-log-viewer (src/flightlog_parser.js)
 * https://github.com/betaflight/blackbox-log-viewer
 * Original work Copyright (C) Nicholas Sherlock and contributors, licensed GPL-3.0.
 *
 * Changes made in this port (per GPL-3.0 section 5a, noted here):
 *  - Converted from an ES module to a plain global script (no build step).
 *  - Removed the `semver` npm dependency; version comparisons now use
 *    BBLTools.versionGte() (src/tools.js).
 *  - Removed GPS (G) and GPS-home (H) frame support entirely. This project's
 *    hardware never logs GPS data, so that code path was dead weight.
 *  - Removed the dependency on flightlog_fields_presenter.js (display/unit
 *    formatting) and flightlog_fielddefs.js's adjustFieldDefsList() (legacy
 *    firmware field-renaming) since this project only targets modern
 *    Betaflight and doesn't need historical compatibility shims.
 *  - Field/predictor/encoding parsing logic, frame stream sync/resync logic,
 *    and prediction math are otherwise unchanged from upstream.
 */

// Firmware type constants (from flightlog_fielddefs.js)
const FIRMWARE_TYPE_UNKNOWN = 0;
const FIRMWARE_TYPE_BASEFLIGHT = 1;
const FIRMWARE_TYPE_CLEANFLIGHT = 2;
const FIRMWARE_TYPE_BETAFLIGHT = 3;
const FIRMWARE_TYPE_INAV = 4;

// Event type constants (from flightlog_fielddefs.js)
const FlightLogEvent = Object.freeze({
  SYNC_BEEP: 0,
...
function FlightLogParser(logData) {
  ...
  function translateFieldName(fieldName) {
    ...
    return translation !== undefined ? translation : fieldName;
  }
  ...
  const isModernFilterFirmware = () =>
    (this.sysConfig.firmwareType === FIRMWARE_TYPE_BETAFLIGHT &&
      BBLTools.versionGte(this.sysConfig.firmwareVersion, "3.0.1")) ||
    (this.sysConfig.firmwareType === FIRMWARE_TYPE_CLEANFLIGHT &&
      BBLTools.versionGte(this.sysConfig.firmwareVersion, "2.0.0"));
  ...
  const parseFieldDefinition = (fieldName, fieldValue) => {
    ...
    const frameInfoHandlers = {
      predictor: () => { frameDef.predictor = BBLTools.parseCommaSeparatedString(fieldValue); },
      encoding: () => { frameDef.encoding = BBLTools.parseCommaSeparatedString(fieldValue); },
      name: () => {
      ...
      signed: () => { frameDef.signed = BBLTools.parseCommaSeparatedString(fieldValue); },
    };
  ...
  const csvHandler = (fn, fv) => { this.sysConfig[fn] = BBLTools.parseCommaSeparatedString(fv); };
  ...
  const rcExpoRateHandler = (fn, fv) => {
    if (BBLTools.stringHasComma(fv)) {
      this.sysConfig[fn] = BBLTools.parseCommaSeparatedString(fv);
    } else {
  ...
  const pidArrayPushHandler = (_fn, fv) => {
    const values = BBLTools.parseCommaSeparatedString(fv);
  ...
  HEADER_HANDLERS["superExpoFactor"] = (_fn, fv) => {
    if (BBLTools.stringHasComma(fv)) {
      const expoParams = BBLTools.parseCommaSeparatedString(fv);
  ...
  HEADER_HANDLERS["magPID"] = (_fn, fv) => { this.sysConfig.magPID = BBLTools.parseCommaSeparatedString(fv, 3); };
  ...
  const parseHeaderLine = () => {
    ...
    const fieldName = translateFieldName(
      BBLTools.asciiArrayToString(stream.data.subarray(lineStart, separatorPos)),
    );
    const fieldValue = BBLTools.asciiArrayToString(
    ...
    if (handler) {
      ...
      this.sysConfig.unknownHeaders.push({ name: fieldName, value: fieldValue });
    }
  };
  ...
  const updateFieldStatistics = (frameType, frame) => {
    ...
    for (let i = 0; i < frame.length; i++) {
      if (!fieldStats[i]) {
        fieldStats[i] = { max: frame[i], min: frame[i] };
      } else {
      ...
      if (predictor[i] === FLIGHT_LOG_FIELD_PREDICTOR_INC) {
        ...
        switch (encoding[i]) {
          ...
          case FLIGHT_LOG_FIELD_ENCODING_NEG_14BIT:
            value = -BBLTools.signExtend14Bit(stream.readUnsignedVB());
            break;
  ...
  const applyPrediction = (fieldIndex, predictor, value, current, previous, previous2) => {
    switch (predictor) {
      ...
      case FLIGHT_LOG_FIELD_PREDICTOR_MINMOTOR:
        value = Math.trunc(value) + Math.trunc(this.sysConfig.motorOutput[0]);
        break;
  ...
  function parseInflightAdjustment(data) {
    ...
    data.name = "Unknown";
    data.func = tmp & 127;
    data.value = tmp < 128 ? stream.readSignedVB() : BBLTools.uint32ToFloat(stream.readU32());
  ...
  function parseEventFrame(_raw) {
    ...
    switch (eventType) {
      ...
      case FlightLogEvent.TWITCH_TEST: {
        ...
        lastEvent.data.value = BBLTools.uint32ToFloat(stream.readU32());
        break;
      }
      case FlightLogEvent.LOGGING_RESUME:
...
FlightLogParser.prototype.FLIGHT_LOG_START_MARKER = BBLTools.asciiStringToByteArray(
